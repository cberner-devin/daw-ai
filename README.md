...
For each prompt, Codex receives an isolated writable projection of that file containing the current graph and bounded active regional state, plus the selected range and the checked-in synth contract under `codex/`. Global temporary roots are excluded from the Codex sandbox. The JSON is a stable sound graph: MIDI clips own beat-relative notes; instruments, effects, and modulators expose numeric parameters; and routing publishes explicitly typed MIDI, audio, and control edges. Read-only listening tools can render selected channels and time ranges as Mel-spectrogram PNGs or report dynamics and spectral measurements from DAW-AI's deterministic synth render. Codex works in an edit, listen, and evaluate loop: it forms a musical plan, applies a focused batch through `apply_sound_graph_edits`, listens to the updated graph, compares the result with the request, and repeats as needed. There is no predetermined iteration or tool-call limit; the overall 20-minute request timeout is the loop boundary. The server validates and publishes each successful tool batch as an undoable edit, and the browser refreshes the arrangement and advances its open-ended activity bar while Codex continues working. Published batches remain explicitly incomplete until Codex exits successfully and the server persists the operation's completion marker. Direct Advanced edits and channel creation or deletion use the same persisted graph.

Prompted edits run as asynchronous jobs so reverse proxies never need to hold one request open while Codex works. The browser polls short status requests, fetches each published intermediate project and the completed project, and shows the current phase, applied steps, and elapsed time. Codex may spend up to 20 minutes on an edit; if the project changes before that edit finishes, the result is rejected instead of overwriting newer work.
