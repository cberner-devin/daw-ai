...
- Effects: `add_effect`, `update_effect`, `delete_effect`. Add accepts Surge XT's native Delay, reverb, modulation, distortion, EQ, dynamics, spectral, tape, resonator, and convolution effect types. Effects expose bypass and wet/dry mix, are addressed by stable ID after creation, and delete removes the graph object and its routing entry. Factory patches retain their embedded Surge effects; added graph effects occupy available native insert/global slots.
