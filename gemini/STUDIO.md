...
- `clips` are MIDI clips with beat-relative note events. Every event has `time`, `duration`, MIDI `pitch`, and normalized `velocity`. A Surge percussion patch is one pitched instrument, never an implicit General MIDI kit. Each drum track therefore owns one dedicated voice and uses only its published canonical pitch: kick 36, snare 38, closed hat 42, open hat 46, or crash 49. Create separate drum tracks for separate voices. `playback.mode` is either `loop`, which repeats `lengthBeats`, or `once`, which plays the phrase once without wrapping. `sourceStart` is the read-only phase anchor and can precede `start` when an edit retains the right side of a clip.
...
- Tracks: `new_track`, `delete_track`. A new track has its required Surge XT instrument and routing but starts with no MIDI clips. For drums, call `new_track` separately with `drumVoice` `kick`, `snare`, `closedHat`, `openHat`, or `crash`; do not place several drum sounds on one track.
